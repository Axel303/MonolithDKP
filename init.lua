...
local function extractItemId(msg)
...
nt, msg)
    ...
    local availableInstanceIId = { 249, 409, 469, 531, 533 };
...
MonDKP.Commands = {
    ...
    ["awardauto"] = function(itemId, playerName, cost)
        if core.IsOfficer and core.Initialized then
            MonDKP:StatusVerify_Update()
            local _, itemLink = GetItemInfo(itemId);
            MonDKP:AwardConfirm(playerName, cost, MonDKP_DB.bossargs.LastKilledBoss, MonDKP_DB.bossargs.CurrentRaidZone, itemLink)
        else
            MonDKP:Print(L["NOPERMISSION"])
        end
    end,
...
function MonDKP_OnEvent(self, event, arg1, ...)
    ...
    elseif event == "CHAT_MSG_LOOT" then
        MonDKP:CheckOfficer();
        if core.IsOfficer then
            handleChatLoot(self, event, arg1)
        end
    end
end
