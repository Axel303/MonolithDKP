...
local function RightClickMenu(self)
	...
	menu = {
		...
		{ text = "Rerolls", notCheckable = true, hasArrow = true,
		  menuList = {
			  { text = "Add a reroll", notCheckable = true, disabled = not core.IsOfficer, func = function()
				  local playerName;
				  for i = 1, #core.SelectedData do
					  playerName = core.SelectedData[i].player;
				  end
				  StaticPopupDialogs["ADD_REROLL"] = {
					  text = "Please add the name of the reroll for " .. playerName,
					  button1 = L["ADD"],
					  button2 = L["CANCEL"],
					  ...
					  OnAccept = function(self, data, data2)
						  local rerollName = self.editBox:GetText();
				  ...
				  StaticPopup_Show("ADD_REROLL")
			  end
			  },
			  { text = "View rerolls", notCheckable = true, func = function()
				  local playerName;
				  for i = 1, #core.SelectedData do
					  playerName = core.SelectedData[i].player;
				  end
				  ...
				  if search then
					  if MonDKP_DKPTable[search[1][1]].rerolls then
						  local rerolls = "";
						  for k, v in pairs(MonDKP_DKPTable[search[1][1]].rerolls) do
							  rerolls = rerolls .. v .. "," .. "\n"
						  end
						  StaticPopupDialogs["ADD_REROLL"] = {
							  text = rerolls,
						  ...
						  StaticPopupDialogs["NO_REROLLS"] = {
							  text = "This player don't have rerolls",
							  button1 = L["CLOSE"],
							  timeout = 0,
							  whileDead = true,
							  preferredIndex = 3,
						  }
						  StaticPopup_Show("NO_REROLLS")
					  end
				  end
			  end
			  }
